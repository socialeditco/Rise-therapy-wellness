...
function applyContentEnhancements() {
  ...
  const start = $('.nav-actions a[href="/start-here"]');
  if (start) start.textContent = 'Start Here';

  const request = $('.nav-actions a[href="/request-evaluation"]');
  if (request) request.textContent = 'Request Eval';

  $$('.audience-card').forEach((card) => {
    if (!card.textContent.includes('Teens')) return;
    const img = $('img', card);
    if (!img) return;
    img.src = 'assets/teen-therapy.jpg';
    img.alt = 'Teen therapy support session';
    img.loading = 'lazy';
    img.decoding = 'async';
  });

  const packet = $('.packet-card img');
  if (packet) {
    packet.src = 'assets/welcome-packet-fallback.svg';
    packet.alt = 'Rise welcome packet and branded materials';
    packet.loading = 'lazy';
    packet.decoding = 'async';
    packet.onerror = null;
  }
}
...
function injectOverrides() {
  ...
  style.textContent = `
    ...
    .proposal-ribbon,.site-header,.utility-bar,.main-nav>*{will-change:opacity,transform}
    .hero-nav-cleared .hero:not(.hero-complete) .hero-pin{top:0!important;height:100svh!important}

    .main-nav{grid-template-columns:clamp(190px,16vw,235px) minmax(510px,1fr) auto!important;gap:clamp(.8rem,1.1vw,1.35rem)!important;padding:.9rem 0!important}
    .brand img{width:clamp(185px,15vw,225px)!important}
    .nav-menu{gap:clamp(.7rem,1vw,1.15rem)!important;justify-content:center!important}
    .nav-menu a{white-space:nowrap!important;font-size:clamp(.9rem,.95vw,1.02rem)!important;letter-spacing:.01em!important}
    .nav-actions{gap:.75rem!important}
    .nav-actions .btn{width:158px!important;min-width:158px!important;min-height:68px!important;padding:.82rem .95rem!important;text-align:center!important;line-height:1.15!important}
    ...
    .demo-pop{z-index:2147483647!important;cursor:pointer!important}
    .demo-pop.active{pointer-events:auto!important}

    .trust-stage{overflow:visible!important;padding:4.8rem 1.45rem 1.55rem!important}
    .trust-carousel{overflow:hidden!important;touch-action:pan-y!important;cursor:grab!important;-webkit-mask-image:linear-gradient(90deg,transparent,#000 7%,#000 93%,transparent);mask-image:linear-gradient(90deg,transparent,#000 7%,#000 93%,transparent)}
    .trust-carousel.dragging{cursor:grabbing!important}
    .trust-track{display:flex!important;gap:1rem!important;will-change:transform!important;transform:translate3d(0,0,0);backface-visibility:hidden;user-select:none!important}
    .review-card{flex:0 0 clamp(330px,30vw,430px)!important;min-height:265px!important}
    .trust-pop{z-index:6!important;pointer-events:none!important;filter:drop-shadow(0 16px 24px rgba(38,70,83,.15))!important;animation:floaty 7s ease-in-out infinite!important}
    .trust-sun-rays{right:4%!important;top:-44px!important;width:210px!important;opacity:.32!important}
    .trust-unicorn-img{left:3%!important;bottom:-18px!important;width:118px!important;transform:rotate(-6deg)!important}
    .trust-ball-img{right:8%!important;bottom:-10px!important;width:94px!important;animation-delay:-2s!important}
    .trust-blocks-img{left:30%!important;top:34px!important;width:82px!important;animation-delay:-3s!important}
    .trust-stars-img{right:31%!important;top:62px!important;width:88px!important;opacity:.72!important;animation-delay:-1s!important}
    .packet-card img{display:block!important;width:100%!important;height:320px!important;object-fit:cover!important;background:#fffdf7!important}

    .pricing-section{background:linear-gradient(180deg,#fffdf7 0%,#fff4e3 52%,#eef8f8 100%)!important}
    .pricing-grid{display:grid!important;grid-template-columns:repeat(3,1fr)!important;gap:1rem!important;align-items:stretch!important}
    .pricing-card{position:relative!important;background:rgba(255,255,255,.92)!important;border:1px solid rgba(38,70,83,.12)!important;border-radius:34px!important;box-shadow:var(--soft-shadow)!important;padding:1.45rem!important;display:flex!important;flex-direction:column!important;gap:.75rem!important}
    .pricing-card.recommended{border:2px solid rgba(241,90,36,.55)!important;transform:translateY(-10px)!important;box-shadow:0 24px 70px rgba(241,90,36,.16)!important}
    .recommend-badge{position:absolute!important;top:-16px!important;right:22px!important;background:var(--orange)!important;color:#fff!important;border-radius:999px!important;padding:.45rem .8rem!important;font-weight:900!important;font-size:.78rem!important;letter-spacing:.08em!important;text-transform:uppercase!important}
    .pricing-kicker{color:var(--orange)!important;font-weight:900!important;letter-spacing:.11em!important;text-transform:uppercase!important;font-size:.78rem!important}
    .pricing-card h3{font-family:"League Spartan"!important;color:var(--navy)!important;font-size:clamp(2rem,2.4vw,2.65rem)!important;line-height:.95!important;margin:0!important}
    .pricing-card .price{font-family:"League Spartan"!important;font-size:clamp(2.6rem,4vw,4rem)!important;font-weight:900!important;color:var(--navy)!important;line-height:.9!important}
    .pricing-card .scope{font-weight:900!important;color:var(--orange)!important;margin:0!important}
    .pricing-card p{color:var(--muted)!important;margin:0!important}
    .pricing-card ul{margin:.35rem 0 0!important;padding-left:1.1rem!important;color:var(--text)!important}
    .pricing-card li{margin:.42rem 0!important}
    .pricing-table-wrap{margin-top:1.25rem!important;background:#fff!important;border:1px solid rgba(38,70,83,.12)!important;border-radius:34px!important;box-shadow:var(--soft-shadow)!important;padding:1.35rem!important;overflow-x:auto!important}
    .pricing-table-wrap h3{font-family:"League Spartan"!important;font-size:2.1rem!important;color:var(--navy)!important;margin:0 0 .8rem!important}
    .pricing-table{width:100%!important;border-collapse:separate!important;border-spacing:0!important;min-width:860px!important}
    .pricing-table th{background:var(--navy)!important;color:#fff!important;text-align:left!important;padding:.9rem!important;font-weight:900!important}
    .pricing-table td{padding:.85rem .9rem!important;border-bottom:1px solid rgba(38,70,83,.1)!important;color:var(--text)!important}
    .pricing-table tr:last-child td{border-bottom:0!important}
    .pricing-table td:first-child{font-weight:900!important;color:var(--navy)!important}
    .pricing-table th:nth-child(3),.pricing-table td:nth-child(3){background:rgba(241,90,36,.07)!important}

    @media(max-width:1200px){
      .main-nav{grid-template-columns:190px 1fr auto!important}
      .nav-actions .btn{width:146px!important;min-width:146px!important}
      .nav-menu{gap:.62rem!important}
      .nav-menu a{font-size:.9rem!important}
    }
    @media(max-width:1100px){
      .main-nav{grid-template-columns:210px 1fr!important}
      .nav-actions .btn{width:auto!important;min-width:0!important}
      .intro .three-up{grid-template-columns:1fr!important;max-width:100%!important}
      .hero.hero-complete .hero-photo-card{width:min(560px,47vw)!important}
      .hero.hero-complete h1{font-size:clamp(3.35rem,6.4vw,5.7rem)!important}
    ...
    @media(max-width:720px){
      ...
      .trust-stage{padding:4.1rem .9rem 1rem!important;overflow:hidden!important}
      .trust-carousel{-webkit-mask-image:none!important;mask-image:none!important}
      .review-card{flex-basis:86vw!important}
      .trust-sun-rays{width:160px!important;top:-28px!important}
      .trust-unicorn-img{width:88px!important}
      .trust-ball-img,.trust-blocks-img,.trust-stars-img{width:66px!important}
      .packet-card img{height:260px!important}
      .pricing-card{padding:1.1rem!important}
      .pricing-table-wrap{padding:1rem!important}
      .pricing-table{min-width:780px!important}
    }
  `;
...
function createRain() {
  ...
  for (let i = 0; i < 82; i += 1) {
    ...
    drop.style.animationDuration = `${0.75 + Math.random() * 0.55}s`;
...
function initHeroAnimation() {
  ...
  if (!window.gsap || !window.ScrollTrigger || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    ...
    if (window.gsap) {
      ...
      gsap.set(headerEls(), { autoAlpha: 1, yPercent: 0 });
  ...
  gsap.set('.logo-sun-glow', { opacity: 0, scale: 0.45 });
  ...
  tl.to('.storm-a', { xPercent: -100, yPercent: -8, opacity: 0.08, duration: 1.4 }, 0)
    .to('.storm-b', { xPercent: 96, yPercent: -8, opacity: 0.08, duration: 1.4 }, 0)
    .to('.rain-layer', { opacity: 0.16, duration: 1.2 }, 0.05)
    .to('.bright-bg', { opacity: 1, duration: 1.3 }, 0.35)
    .to('.clear-weather', { opacity: 1, duration: 1.25 }, 0.72)
    .fromTo('.white-a', { xPercent: -20, opacity: 0 }, { xPercent: 0, opacity: 0.95, duration: 1.05 }, 0.9)
    .fromTo('.white-b', { xPercent: 20, opacity: 0 }, { xPercent: 0, opacity: 0.95, duration: 1.05 }, 1.0)
    .to('.rainbow-img', { opacity: 0.95, y: 0, duration: 1.2 }, 1.35)
    .to('.hero-sun-mark', { opacity: 1, y: 0, scale: 1, duration: 1.05 }, 1.9)
    .to('.logo-sun-glow', { opacity: 1, scale: 1.45, duration: 1.1 }, 2.05)
    .to('.hero-rise-wordmark', { opacity: 1, y: 0, duration: 0.95 }, 2.55)
    .to('.hero-brand-subline', { opacity: 1, y: 0, duration: 0.72 }, 2.95)
    ...
    .to(['.hero-sun-mark', '.hero-rise-wordmark', '.hero-brand-subline', '.logo-sun-glow'], { opacity: 0, duration: 0.95 }, 4.0)
    .to(['.storm-bg', '.storm-clouds', '.clear-weather', '.rain-layer'], { opacity: 0, duration: 0.95 }, 4.0);
  ...
  ScrollTrigger.create({
    ...
    onLeave: (self) => {
      tl.progress(1);
      requestAnimationFrame(() => completeHero(hero, pin, self));
    }
  });
}
...
function initPrototypeBubbles() {
  ...
  const bubbleTarget = (node) => {
    const link = node?.closest?.('a,button,[role="button"]');
    if (!link) return null;
    if (link.closest('#demoPop')) return null;
    if (link.classList.contains('bubble-link') || link.classList.contains('page-link') || link.classList.contains('btn')) return link;
    const href = link.getAttribute('href') || '';
    if (href.startsWith('/') || href.startsWith('#')) return link;
    return null;
  };

  const bubbleText = (link) => {
    const label = (link.textContent || 'Prototype link').trim().replace(/\s+/g, ' ');
    return {
      title: link.dataset.bubbleTitle || label || 'Prototype link',
      body: link.dataset.bubble || 'In the finished website, this would route visitors to the appropriate page, section, form, or external workflow.'
    };
  };
  ...
  const show = (link, event) => {
    ...
    if (isMobile()) {
      ...
      const x = Math.min(Math.max(rect.left + rect.width / 2, 220), window.innerWidth - 220);
      const y = Math.max(rect.top - 12, 92);
      pop.style.left = `${x}px`;
      pop.style.top = `${y}px`;
  ...
  document.addEventListener('click', (event) => {
    if (event.target.closest('#demoPop')) {
      hide();
      return;
    }
    const link = bubbleTarget(event.target);
